...
# For example: t_expo_1 = 1000, t_buri_1 = 100, t_expo_1 = 10 means:
...
def expo(x, order, sigma_phi, mu, t_expo_1):
    ...
    return ((order - 1) * sigma_phi * t_expo_1 * np.exp(-mu * x) + 1)**(1 \
        / (1 - order))
...
def expo_buri(x, order, sigma_phi, mu, f, t_expo_1, t_buri_1):
    ...
    L_initial_condition = expo(x, order, sigma_phi, mu, t_expo_1)

    # burial works same for single and general order kinetics
    return 1 - (1 - L_initial_condition) * np.exp(-f * t_buri_1)


def expo_buri_expo(x, order, sigma_phi, mu, f, t_expo_1, t_buri_1, t_expo_2):
    ...
    Args:
        ...
        f (float): f = D*(x) / D_0, charge filling rate in the rock, here
            assumend as a constant.
        t_expo_1 (float): First exposure time of the rock surface.
        t_buri_1 (float): Burial time of the rock surface.
        t_expo_2 (float): Second exposure time of the rock surface.
    ...
    # get initial condition for the luminescence profile
    L_initial_condition = expo_buri(x, order, sigma_phi, mu, f, t_expo_1,
        t_buri_1)

    # check if order of the model is 1 -> single order kinetics
    if np.isclose(order, 1.0, rtol=1e-9, atol=0.0):
        return L_initial_condition * np.exp(-sigma_phi * t_expo_2 \
            * np.exp(-mu * x))
    # else general order kinetics
    return L_initial_condition * ((order - 1) * sigma_phi * t_expo_2 \
