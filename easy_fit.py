...
def easy_fit(x_data, y_data, model_function, known_params, initial_guess,
    y_err_sigma=None, only_positive=True):
    ...
    if y_err_sigma is not None:
        y_err_sigma = np.asarray(y_err_sigma)
    ...
    user_params = set(initial_guess.keys()).union(set(known_params.keys()))
    model_params = set(all_param_names)

    # if they don't match throw an exception
    if user_params != model_params:
        missing = model_params - user_params
        extra = user_params - model_params
        msg = []
        if missing:
